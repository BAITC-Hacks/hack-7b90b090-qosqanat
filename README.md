...
В начале подключения микрофон измеряет фон в течение 300 мс. После 700 мс паузы приложение завершает распознавание реплики и запускает ответ. Уровень фона уточняется во время разговора. Короткий щелчок не сбрасывает отсчёт паузы. Отключение микрофона завершает текущую реплику и сохраняет её текст. Во время подготовки и озвучивания ответа для перебивания нужны 160 мс устойчивой речи. Длинная пауза внутри вопроса тоже завершает реплику.

В консоли браузера доступны отметки `voice.transition`: окончание речи, финальный текст, начало ответа и поступление звука. Они содержат название этапа и время. Отметки `voice.vad` показывают уровень сигнала и порог паузы. Текст разговора и аудио в эти отметки не попадают. Если приложение не смогло завершить распознавание реплики, оно показывает ошибку в интерфейсе. Незавершённый текст не подтверждает операции.
